...
def set_reaction_arrows():
    ...
    # 5. 筛选 relationship 为 reaction(s) 的边
        reactions_edges = edge_table[edge_table.get('relationship') == 'reaction(s)']
        
        if reactions_edges.empty:
            print("❌ 没有找到 relationship 为 'reaction(s)' 的边")
            return False
        
        print(f"✓ 找到 {len(reactions_edges)} 条 relationship 为 'reaction(s)' 的边")
        
        # 6. 设置单向箭头样式
        reactions_names = reactions_edges['SUID'].tolist()
        
        aa = p4c.get_line_styles()
        print(f"类型{aa}")
        
        # 设置目标箭头为单向箭头
        p4c.set_edge_property_bypass(
            edge_names=reactions_names,
            visual_property='EDGE_TARGET_ARROW_SHAPE',
            new_values='NONE'
        )
        ...
        print("✓ 已成功将 reaction(s) 关系的边设置为单向箭头")
        print("  - 目标箭头: NONE")
        print("  - 源箭头: NONE")
        print("  - 线条类型: SEPARATE_ARROW")
