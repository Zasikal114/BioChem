...
def fill_empty_cells_with_zero(file_path, output_path=None, sheet_name=None):
    """
    将Excel中有内容的行列中的空单元格替换为0
    
    参数:
    file_path: Excel文件路径
    output_path: 输出文件路径（默认为原文件名加上_filled）
    ...
    try:
        ...
        if output_path is None:
            if '.' in file_path:
                ...
                output_path = f"{name_parts[0]}_filled.{name_parts[1]}"
            else:
                output_path = f"{file_path}_filled.xlsx"
...
def fill_empty_cells_advanced(file_path, output_path=None, sheet_name=None, fill_value=0):
    """
    高级版本：提供更多选项的空单元格填充功能
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        fill_empty_cells_with_zero(input_file, output_file, sheet)
    else:
        print("""
使用方法:
1. 修改脚本中的 file_path 变量为您的Excel文件路径
2. 或者通过命令行运行: python script.py <输入文件> [输出文件] [工作表名]

示例:
python excel_fill_empty.py data.xlsx
python excel_fill_empty.py data.xlsx result.xlsx Sheet1
        """)